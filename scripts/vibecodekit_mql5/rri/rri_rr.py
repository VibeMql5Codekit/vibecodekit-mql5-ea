...
RISK_KEYWORDS = (
    "risk", "reward", "drawdown", "stop", "loss", "profit", "lot",
    "correlation", "margin", "exposure", "portfolio", "heat",
)


def build_risk_reward_review(mode: str = "TEAM", persona: str = "all") -> dict:
    ...
    for name in names:
        ...
        focused = [
            ...
            if any(keyword in question["text"].lower() for keyword in RISK_KEYWORDS)
        ]
        reviews.append({
            "persona": name,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="RRI risk-reward review")
    ...
    result = build_risk_reward_review(args.mode, args.persona)
    if args.json:
        print(json.dumps(result, indent=2))
    else:
        print(f"RRI Risk-Reward [{args.mode}]: {result['total_questions']} question(s)")
